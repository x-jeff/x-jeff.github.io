...
到目前为止，我们使用过的仅有的两种可调用对象是[函数](http://shichaoxin.com/2021/12/04/C++基础-第三十四课-函数基础/)和[函数指针](http://shichaoxin.com/2022/04/25/C++基础-第四十课-函数指针/)。还有其他两种可调用对象：重载了函数调用运算符的类（后续博文会有详细介绍），以及**lambda表达式（lambda expression）**。
