...
高斯混合聚类算法描述见上图。以第2部分的数据集为例，令高斯混合成分的个数$k=3$。算法开始时，假定将高斯混合分布的模型参数初始化为：$\alpha_1 = \alpha_2 = \alpha_3 = \frac{1}{3}$；$\mathbf{\mu}_1 = \mathbf{x}_6, \mathbf{\mu}_2 = \mathbf{x}_{22}, \mathbf{\mu}_3 = \mathbf{x}_{27}$；$\Sigma_1=\Sigma_2=\Sigma_3=\begin{pmatrix} 0.1 & 0.0 \\ 0.0 & 0.1 \\ \end{pmatrix}$。
