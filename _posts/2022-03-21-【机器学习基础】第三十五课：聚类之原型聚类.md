...
换言之，$p\_{\mathcal{M}}(z\_j=i \mid \mathbf{x}\_j)$给出了样本$\mathbf{x}_j$由第$i$个高斯混合成分生成的后验概率。为方便叙述，将其简记为$\gamma_{ji}(i=1,2,...,k)$。
...
高斯混合聚类算法描述见上图。以第2部分的数据集为例，令高斯混合成分的个数$k=3$。算法开始时，假定将高斯混合分布的模型参数初始化为：$\alpha\_1 = \alpha\_2 = \alpha\_3 = \frac{1}{3}$；$\mathbf{\mu}\_1 = \mathbf{x}\_6, \mathbf{\mu}\_2 = \mathbf{x}\_{22}, \mathbf{\mu}\_3 = \mathbf{x}\_{27}$；$\Sigma\_1=\Sigma\_2=\Sigma\_3=\begin{pmatrix} 0.1 & 0.0 \\ 0.0 & 0.1 \\ \end{pmatrix}$。
