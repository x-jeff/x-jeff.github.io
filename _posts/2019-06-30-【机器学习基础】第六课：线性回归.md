...
* $w^*,b^*$表示$w$和$b$的解。
* $\arg\min$：就是使后面这个式子达到最小值时的变量的取值；$\arg\min$：就是使后面这个式子达到最大值时的变量的取值。（⚠️达到最小（大）值时，变量的取值可能有多个。）
...
令式(3.2)等于零便可得到$\widehat{\mathbf w}$最优解的闭式解。由于计算过于复杂，所以这里讨论一种简单的情况，即当$\mathbf X^T \mathbf X$为[满秩矩阵](wait)或[正定矩阵](wait)时，令式(3.2)等于零可得：
