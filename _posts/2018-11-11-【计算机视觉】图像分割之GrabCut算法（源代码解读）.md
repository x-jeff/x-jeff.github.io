...
~~~
如果像素点$p=GC\_BGD$或者$p=GC\_PR\_BGD$，则执行$bgdGMM.whichComponent(color)$，返回点$p$最有可能属于背景$GMM$模型中的哪个高斯分量。反之，如果像素点$p=GC\_FGD$或者$p=GC\_PR\_FGD$，则执行$fgdGMM.whichComponent(color)$，返回点$p$最有可能属于前景$GMM$模型中的哪个高斯分量。
