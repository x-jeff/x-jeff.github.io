...
式1~式4中的$d_* (P)$（$\*$为$x,y,h,w$中的一个）为一个线性模型，自变量为CNN网络中POOL5层的输出，表示为$\phi\_5(P)$。则$d\_\* (P)=w\_\* ^T \phi\_5 (P)$。参数$w\_\*$的学习方法为岭回归（即改良的[最小二乘估计](http://shichaoxin.com/2019/06/30/机器学习基础-第六课-线性回归/#21最小二乘法)，其实就是多加了个正则化项）：aaa
